...
public class HyperRectangle {
    ...
    public Interval get_interval(int at_dim){
        ...
        if (at_dim <= dim){
            ...
            throw new Error("empty HyperRectangle");
        }
    ...
    public boolean contains(HyperRectangle inside, boolean printErrors){
        ...
        if (dim != inside.dim){
            throw new Error("two rectangles do not have the same dimensions");
        }
        else{
    ...
    public HyperRectangle convex_hull(HyperRectangle contained){
        ...
        if (this.dim != contained.dim){
            throw new Error("Error: Dimension inconsistency to make a convex hull");
        }
        else{
    ...
    public boolean check_intersect(UnsafeSet unsafe_set){
        ...
        for (Integer d: dims){
            ...
            if ((d > this.dim) || (d < 0)){
                throw new Error("Invalid dimension for checking safety");
            }
            else{
